...
import testStreamImg from "../../assets/images/test.PNG"

const PlayPage = props => {
  const { ipaddress, selkeys, spd, isConnect, startConnection = () => { }, endConnection = () => { }, startKeyLoop, stopKeyLoop, isLoading } = props;
  return (
    <div className="play-page">
      <div className="play-page_camera">
        {!isConnect ? <div className="play-page_camera_none">
          <div className="play-page_camera_none_image">
            <NoCamImg />
          </div>
          <div className="play-page_camera_none_connect">
            <Button isLoading={isLoading} onClick={() => startConnection(ipaddress)}>Connect</Button>
          </div>
        </div> : <div className="play-page_camera_ok">
            <div className="play-page_camera_ok_image">
              <img src={testStreamImg} alt="webcam not working" />
            </div>
            <div className="play-page_camera_ok_disconnect">
              <Button isLoading={isLoading} status="danger" onClick={endConnection}>X</Button>
            </div>
          </div>}
      </div>
